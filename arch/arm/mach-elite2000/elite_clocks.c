...
int elite_cpu_set_voltage(struct clk *c, unsigned int voltage_idx)
{
    ...
    if(elite_rail->disabled)
       return -ENOSYS;
        ...
        dvfs_rail_set_voltage(elite_rail);
 	return 0;
}
...
void elite_switch_pll(int switch_pll_cmd)
{
    ...
    case CPUPLLCMD_PARKING:
	 ...
	 cpupll_test_ctrl.val = pmu_readl(DVFS_CPU_PLL_TEST_CTRL);
         cpupll_test_ctrl.field.sel = CPU_PLL_SEL_PARKING;
	 pmu_writel(cpupll_test_ctrl.val,DVFS_CPU_PLL_TEST_CTRL);
         break;
    case CPUPLLCMD_CPUPLL:
         cpupll_test_ctrl.val = pmu_readl(DVFS_CPU_PLL_TEST_CTRL);
         cpupll_test_ctrl.field.sel = CPU_PLL_SEL_CPUPLL;
	 pmu_writel(cpupll_test_ctrl.val,DVFS_CPU_PLL_TEST_CTRL);
         break;
    default:
         BUG();
...
static int elite_cpu_clk_set_rate(struct clk *c, unsigned long rate)
{
        ...
        elite_cpu_set_rate(c, idx);
        printk(KERN_DEBUG "*****%s: CPU CLK is set to %ld ********************** \n",__func__,rate);
        if (ret)
...
static const struct of_device_id matches[] __initconst = {
	{ .compatible = "s3graphics,elite2000-pmc" },
	{ }
};
#endif
...
//#ifdef CONFIG_OF
	if (of_have_populated_dt()) {
		...
		if (np) {
			np = of_find_compatible_node(NULL, NULL, "s3graphics,elite2000-pmc");
