...
#ifdef CONFIG_PERF_EVENTS
void counter_output(const char *name, s64 count, int cpu)
{
    trace_s3_counter_output(name,count, cpu);
}
#endif
...
void elite_set_pg_timer(unsigned int us,int cpu)
{
  ...
  case cpu0:
       ...
       pmu_ptimer_ctrl_reg_addr += PMU_CORE0_PTIMER_CTRL_REG; 
       break;
  case cpu1:
...
unsigned int elite_power_gating(unsigned int us,struct cpuidle_device *dev)
{
	...
	//return remain;
     pg_dbg_addr = (void __iomem *)ELITE_PMU_BASE + 0xc018;
...
//	    remain =elite_pg_timer_remain(cpu);
...
//;elite1k-520016c-YSW-13	printk("max cpu lock over\n");
...
void elite_pmu_reset(char mode, const char *cmd)
{
	...
	if (cmd) { //inform bootloader to enter Android recovery mode
		...
		if (!strcmp(cmd, "recovery")) {
			// using Vsus power domain GENERAL_CFG1_WAKEUP register
			// to pass message to bootloader
...
#define HAM_NEON_PGREF_LOADING 0x8000000
#define HAMCNTEN 1<<5
...
void elite_gic_affinity_to_all(unsigned int irq)
{
    ...
    if(irq==ELITE_NEON0_MONITOR_CORES_IRQ)
        val = (val & ~mask) | (1 << shift); //target to cpu0
    else if(irq==ELITE_NEON1_MONITOR_CORES_IRQ)
