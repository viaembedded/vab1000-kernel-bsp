...
#else
static int DbgOut(const char* fmt, ...)
{
    return 0;
}
#endif
...
DECLARE_WAIT_QUEUE_HEAD(DmxWaitQ);
static int flag = 0;

static int dmx_major = 234;
static int dmx_minor = 0;
static int int_status = 0;
static int number_of_devices = 1;
struct class *dmx_class;
PDMX_REGS pdmx_regs;
...
static irqreturn_t int_top_half_handler(int irq, void *handle)
{
#ifndef LINUX_SIM
...
static long dmx_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case IOCTL_SOFT_RESET:
			...
			/*
			while((pdmx_regs->DBGREG[1]&0xF00000)!=0x100000 && (pdmx_regs->DBGREG[1]&0xF00000)!=0x200000)
			{
				udelay(10);
				i++;
			}
			*/
			udelay(100);
			//printk(KERN_INFO "wait for %d times\n", i);
			}
...
struct file_operations dmx_fops = 
{
	.owner			=	THIS_MODULE,
	.open			  =	dmx_open,
	.release		=	dmx_release,
