...
{
	int ret;
	printk("slim_match_ctrl_to_boardinfo ctrl->nr=%d,bi->bus_num=%d\n",ctrl->nr,bi->bus_num);
...
int slim_register_board_info(struct slim_boardinfo const *info, unsigned n)
{
	...
	if (!bi)
		return -ENOMEM;
	printk("slim_register_board_info n=%d\n",n);
