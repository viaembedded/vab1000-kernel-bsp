...
static const struct net_device_ops ks_netdev_ops = {
	...
	.ndo_set_rx_mode = ks_wlan_set_multicast_list, //.ndo_set_multicast_list = ks_wlan_set_multicast_list,
...
int ks_wlan_update_phy_information(ks_wlan_private *priv)
{
	...
	//printk(KERN_ERR  "in_interrupt = %ld\n", in_interrupt());
...
int ks_wlan_read_config_file(ks_wlan_private *priv)
{
...
#ifndef NO_FIRMWARE_CLASS
	const struct ks_firmware *fw_entry = NULL;
	...
	unsigned char *cur_p, *end_p, *wk_p;
	char wk_buff[256];
	...
	printk(KERN_INFO  "wlan_net : start copy rom file\n");
	strcpy(&priv->reg.rom_file[0], ROM_FILE);
	printk(KERN_INFO  "wlan_net : copy rom file ok\n");
	priv->skb = NULL;
	...
	priv->reg.tx_rate = TX_RATE_FULL_AUTO;
	priv->reg.rate_set.size = 12;
	printk(KERN_INFO  "wlan_net : set the config\n");
#ifndef NO_FIRMWARE_CLASS
#if (defined _PCMCIA_)
...
#endif
#if 0
	fw_entry = kzalloc(sizeof(struct firmware), GFP_KERNEL);
	if(!fw_entry)
	{	
		printk(KERN_ERR "error to zalloc config\n");
		return 1;
	}
#endif

	if(request_firmware((const struct firmware **)&fw_entry, cfg_file, dev)!=0){
		printk(KERN_ERR  "error request_firmware() file=%s\n", cfg_file);
		return 1;
	}

	printk(KERN_ERR "success request_firmware() file=%s size=%d\n,data ptr%u", cfg_file, fw_entry->size, (unsigned int)fw_entry->data);

	cur_p = (u8 *)fw_entry->data;
	...
	orgfs=get_fs();
	set_fs(KERNEL_DS);
	printk(KERN_INFO  "wlan_net : set the dev\n");
	return 1;
	...
	if (IS_ERR(srcf)) {
		...
		printk(KERN_ERR "ok %ld opening %s\n", -PTR_ERR(srcf),cfg_file);
	...
	cfg_buf = (char *)kzalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC);
        ...
        printk(KERN_ERR "read : read config\n");
	printk(KERN_ERR "read retval=%d  file=%s\n", nr_read, priv->reg.cfg_file);
	retval=filp_close(srcf ,NULL);
	if (retval)
		printk(KERN_ERR "error %d closing %s\n", -retval,priv->reg.cfg_file);
...
#endif
//	*end_p = '\0';
	printk(KERN_INFO  "wlan_net : end_p\n");
	...
	while (cur_p < end_p) {
		...
		if ((*cur_p == '#') || (cfg_tbl[i].key_len == 0)) {
			...
			case  4: /* "PowerManagementMode","POWER_ACTIVE" */
				...
				printk(KERN_INFO  "wlan_net : 4\n");
				break;
			case  5: /* "RTSThreshold","2347" */
				j = atoi(wk_buff);
				priv->reg.rts = (unsigned long)j;
				printk(KERN_INFO  "wlan_net : 5\n");
				break;
			case  6: /* "SSID","" */
				if (*wk_p != '"')
					break;
				...
				wk_p++;
				printk(KERN_INFO  "wlan_net : 6\n");
				break;
			case  7: /* "TxRate","Auto" */
				rate_set_configuration(priv, wk_p);
				printk(KERN_INFO  "wlan_net : 7\n");
				break;
...
{
	ks_wlan_private *priv = (ks_wlan_private *)dev->ml_priv;

	if (priv->sleep_mode == SLP_SLEEP){ return -EPERM; }  /* for SLEEP MODE */
	//printk(KERN_INFO "[wlan]ks_wlan_get_name  %u.\n",priv->dev_state);	
			    ...
			    struct iw_freq *fwrq, char *extra)
{
	...
	if((fwrq->m > 1000) || (fwrq->e > 0))
		...
		if((channel < 1) || (channel > 14)) {
			printk(KERN_ERR  "%s: New channel value of %d is invalid!\n", dev->name, fwrq->m);
			rc = -EINVAL;
		} else {
			/* Yes ! We can set it !!! */
			   ...
			   struct sockaddr *ap_addr, char *extra)
{
	...
	printk(KERN_ERR  "bssid = %02x:%02x:%02x:%02x:%02x:%02x\n",
			    ...
			    struct iw_param *vwrq, char *extra)
{
	...
	//printk(KERN_ERR  "in_interrupt = %ld update_phyinfo = %d\n", 
			      ...
			      struct iw_point *dwrq, char *extra)
{
	...
	dwrq->flags |= index + 1;
	//printk(KERN_ERR "encoding flag = 0x%04X\n",dwrq->flags);
			     ...
			     struct iw_param *vwrq, char *extra)
{
	ks_wlan_private *priv = (ks_wlan_private *)dev->ml_priv;
	short enabled;
	//printk(KERN_ERR "ks_wlan_set_power\n");
	if (priv->sleep_mode == SLP_SLEEP){ return -EPERM; }  /* for SLEEP MODE */
...
{
	ks_wlan_private *priv = (ks_wlan_private *)dev->ml_priv;
	//printk(KERN_ERR "ks_wlan_get_power\n");
	if (priv->sleep_mode == SLP_SLEEP){ return -EPERM; }  /* for SLEEP MODE */
...
#define RSN_INFO_ELEM_ID 0x30
		...
		if (ap->wpa_ie.id == GENERIC_INFO_ELEM_ID && ap->wpa_ie.size != 0) {
			...
			//printk(KERN_ERR "ap->rsn.size=%d\n",ap->wpa_ie.size);
			//printk(KERN_ERR "iwe.u.data.length=%d\n",iwe.u.data.length);
				 ...
				 struct iw_param *vwrq, char *extra)
{
	...
	//printk(KERN_ERR "ks_wlan_set_auth_mode  index=%d:value=%08X\n",index,value);
				 ...
				 struct iw_param *vwrq, char *extra)
{
	ks_wlan_private *priv = (ks_wlan_private *)dev->ml_priv;
	int index = (vwrq->flags & IW_AUTH_INDEX);
	//printk(KERN_ERR " ks_wlan_get_auth_mode  index=%d\n",index);
				  ...
				  struct iw_point *dwrq, char *extra)
{
	...
	//printk(KERN_ERR "flags=%04X:: ext_flags=%08X\n",dwrq->flags, enc->ext_flags);
				     ...
				     __u32 *uwrq, char *extra)
{
	...
	//printk(KERN_ERR " ks_wlan_set_sleep_mode\n");
				     ...
				     __u32 *uwrq, char *extra)
{
	...
	//printk(KERN_ERR  "GET_SLEEP_MODE %d\n", priv->sleep_mode);
				     ...
				     struct iw_point *dwrq, char *extra)
{
	...
	printk(KERN_ERR "%d(%#x): %02X %02X %02X %02X ... %02X %02X %02X\n", 
...
{
	ks_wlan_private *priv = dev->ml_priv;
...
int ks_wlan_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	//printk(KERN_INFO "test for xmit!\n");
...
int ks_wlan_close(struct net_device *dev)
{
	...
	printk(KERN_ERR  "%s: Shutting down ethercard, status was 0x%4.4x.\n", 
		dev->name, 0x00);
