...
#if 0
static void printf_hif_event(int event){

	switch(event){
	case HIF_DATA_REQ	:
		printk(KERN_INFO "HIF_DATA_REQ\n");
		break;
	case HIF_DATA_IND	:
		printk(KERN_INFO "HIF_DATA_IND\n");
		break;
	case HIF_MIB_GET_REQ	:
		printk(KERN_INFO "HIF_MIB_GET_REQ\n");
		break;
	case HIF_MIB_GET_CONF	:
		printk(KERN_INFO "HIF_MIB_GET_CONF\n");
		break;
	case HIF_MIB_SET_REQ	:
		printk(KERN_INFO "HIF_MIB_SET_REQ\n");
		break;
	case HIF_MIB_SET_CONF	:
		printk(KERN_INFO "HIF_MIB_SET_CONF\n");
		break;
	case HIF_POWERMGT_REQ	:
		printk(KERN_INFO "HIF_POWERMGT_REQ\n");
		break;
	case HIF_POWERMGT_CONF	:
		printk(KERN_INFO "HIF_POWERMGT_CONF\n");
		break;
	case HIF_START_REQ	:
		printk(KERN_INFO "HIF_START_REQ\n");
		break;
	case HIF_START_CONF	:
		printk(KERN_INFO "HIF_START_CONF\n");
		break;
	case HIF_CONNECT_IND	:
		printk(KERN_INFO "HIF_CONNECT_IND\n");
		break;
	case HIF_STOP_REQ	:
		printk(KERN_INFO "HIF_STOP_REQ\n");
		break;
	case HIF_STOP_CONF	:
		printk(KERN_INFO "HIF_STOP_CONF\n");
		break;
	case HIF_PS_ADH_SET_REQ	:
		printk(KERN_INFO "HIF_PS_ADH_SET_REQ\n");
		break;
	case HIF_PS_ADH_SET_CONF:
		printk(KERN_INFO "HIF_PS_ADH_SET_CONF\n");
		break;
	case HIF_INFRA_SET_REQ	:
		printk(KERN_INFO "HIF_INFRA_SET_REQ\n");
		break;
	case HIF_INFRA_SET_CONF	:
		printk(KERN_INFO "HIF_INFRA_SET_CONF\n");
		break;
	case HIF_ADH_SET_REQ	:
		printk(KERN_INFO "HIF_ADH_SET_REQ\n");
		break;
	case HIF_ADH_SET_CONF	:
		printk(KERN_INFO "HIF_ADH_SET_CONF\n");
		break;
	case HIF_AP_SET_REQ	:
		printk(KERN_INFO "HIF_AP_SET_REQ\n");
		break;
	case HIF_AP_SET_CONF	:
		printk(KERN_INFO "HIF_AP_SET_CONF\n");
		break;
	case HIF_ASSOC_INFO_IND	:
		printk(KERN_INFO "HIF_ASSOC_INFO_IND\n");
		break;
	case HIF_MIC_FAILURE_REQ:
		printk(KERN_INFO "HIF_MIC_FAILURE_REQ\n");
		break;
	case HIF_MIC_FAILURE_CONF	:
		printk(KERN_INFO "HIF_MIC_FAILURE_CONF\n");
		break;
	case HIF_SCAN_REQ	:
		printk(KERN_INFO "HIF_SCAN_REQ\n");
		break;
	case HIF_SCAN_CONF	:
		printk(KERN_INFO "HIF_SCAN_CONF\n");
		break;
	case HIF_PHY_INFO_REQ	:
		printk(KERN_INFO "HIF_PHY_INFO_REQ\n");
		break;
	case HIF_PHY_INFO_CONF	:
		printk(KERN_INFO "HIF_PHY_INFO_CONF\n");
		break;
	case HIF_SLEEP_REQ	:
		printk(KERN_INFO "HIF_SLEEP_REQ\n");
		break;
	case HIF_SLEEP_CONF	:
		printk(KERN_INFO "HIF_SLEEP_CONF\n");
		break;
	case HIF_PHY_INFO_IND	:
		printk(KERN_INFO "HIF_PHY_INFO_IND\n");
		break;
	case HIF_SCAN_IND	:
		printk(KERN_INFO "HIF_SCAN_IND\n");
		break;
	case HIF_INFRA_SET2_REQ	:
		printk(KERN_INFO "HIF_INFRA_SET2_REQ\n");
		break;
	case HIF_INFRA_SET2_CONF:
		printk(KERN_INFO "HIF_INFRA_SET2_CONF\n");
		break;
	case HIF_ADH_SET2_REQ	:
		printk(KERN_INFO "HIF_ADH_SET2_REQ\n");
		break;
	case HIF_ADH_SET2_CONF	:
		printk(KERN_INFO "HIF_ADH_SET2_CONF\n");
	}
}
#endif
void ks_wlan_hw_wakeup_task(struct work_struct *work)
{
	...
	if(ps_status==PS_SNOOZE){
		...
		if(!wait_for_completion_interruptible_timeout(&priv->psstatus.wakeup_wait,HZ/50)){ /* 20ms timeout */
			//printk(KERN_INFO  "wake up timeout !!!\n");
			schedule_work(&priv->ks_wlan_wakeup_task);
			return;
		}
	}
	else{
		//printk(KERN_INFO  "ps_status=%d\n",ps_status);
	}

	/* power save */
	if(atomic_read(&priv->sme_task.count) > 0){
		//printk(KERN_INFO  "sme task enable.\n");
...
int ks_wlan_do_power_save(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "psstatus.status=%d\n",atomic_read(&priv->psstatus.status));
...
int get_current_ap(ks_wlan_private *priv, struct link_ap_info_t *ap_info)
{
	...
	//printk(KERN_INFO  "get_current_ap\n");
	ap = &(priv->current_ap);
	...
	if((priv->connect_status & CONNECT_STATUS_MASK)==CONNECT_STATUS){
		...
		printk(KERN_INFO  "IWEVENT: connect bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
...
int get_ap_information(ks_wlan_private *priv, struct ap_info_t *ap_info, struct local_ap_t *ap)
{
	...
	//printk(KERN_INFO  "get_ap_information\n");
	memset(ap,0,sizeof(struct local_ap_t));
	...
	while(bsize > offset){
		...
		case 0: /* ssid */
			if(*(bp+1) <= SSID_MAX_SIZE){
				...
				//printk(KERN_INFO   "size over :: ssid size=%d \n",*(bp+1)); 
		...
		case 50: /* ext rate */
			if((*(bp+1) + ap->rate_set.size) <= RATE_SET_MAX_SIZE){
				...
				//printk(KERN_INFO   "size over :: rate size=%d \n",
		...
		case 48: /* RSN(WPA2) */
			...
			if(*(bp+1) <= RSN_IE_BODY_MAX){
				...
				//printk(KERN_INFO   "size over :: rsn size=%d \n",*(bp+1)); 
				ap->rsn_ie.size = RSN_IE_BODY_MAX;
			}
		...
		case 221: /* WPA */
			if(!memcmp(bp+2, "\x00\x50\xf2\x01", 4)){ /* WPA OUI check */
				...
				if(*(bp+1) <= RSN_IE_BODY_MAX){
					...
					//printk(KERN_INFO   "size over :: wpa size=%d \n",*(bp+1)); 
					ap->wpa_ie.size = RSN_IE_BODY_MAX;
				}
		...
		case 47: /* Reserve ID 47 Broadcom AP */
			break;
		default:
			//printk(KERN_INFO   "unknown Element ID=%d \n",*bp); 
			break;
		}
...
void hostif_data_indication(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_data_indication\n");

	/* min length check */
	if (priv->rx_size <= ETH_HLEN) {
		//printk(KERN_INFO  "rx_size = %d\n", priv->rx_size);
		...
		if(memcmp(&eth_hdr->h_source[0],&priv->eth_addr[0],ETH_ALEN)){ /* source address check */
			...
			if(((auth_type==TYPE_PMK1 && priv->wpa.pairwise_suite == IW_AUTH_CIPHER_TKIP)||
			   ...
			   && priv->wpa.key[auth_type-1].key_len){
				//printk(KERN_INFO  "TKIP: protocol=%04X: size=%u\n", eth_proto, priv->rx_size);
				/* MIC save */
				...
				if(memcmp(michel_mic.Result, RecvMIC, 8)){
					...
					printk(KERN_INFO  "IWEVENT:MICHAELMICFAILURE\n");
	...
	//printk(KERN_INFO "check rx data 00th: %x , 13th %x \n",*(priv->rxp),*(priv->rxp+12));
	...
	case 0xAA: /* SNAP */
		...
		//printk(KERN_INFO  "SNAP, rx_ind_size = %d\n", rx_ind_size);
	...
	case 0xF0: /* NETBEUI/NetBIOS */
		...
		skb = dev_alloc_skb (rx_ind_size);
		//printk(KERN_INFO  "NETBEUI/NetBIOS rx_ind_size=%d\n", rx_ind_size);
...
void hostif_mib_get_confirm(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO   "hostif_mib_get_confirm\n");
	...
	if (mib_status != 0) {
		/* in case of error */
		printk(KERN_INFO   "attribute=%08X, status=%08X\n", mib_attribute, mib_status);
		return;
	}
	...
	case DOT11_MAC_ADDRESS:
		/* MAC address */
		//printk(KERN_INFO  " mib_attribute=DOT11_MAC_ADDRESS\n");
	...
	case DOT11_PRODUCT_VERSION:
		/* firmware version */
		//printk(KERN_INFO  " mib_attribute=DOT11_PRODUCT_VERSION\n");
	...
	case LOCAL_EEPROM_SUM:
		...
		break;
	default:
		printk(KERN_INFO  "mib_attribute=%08x\n",(unsigned int)mib_attribute);
		break;
	}
}
...
void hostif_mib_set_confirm(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_mib_set_confirm\n");
	...
	if (mib_status != 0) {
		/* in case of error */
		//printk(KERN_INFO   "error :: attribute=%08X, status=%08X\n", mib_attribute, mib_status);
	}
		//printk(KERN_INFO   "mib :: attribute=%08X, status=%08X\n", mib_attribute, mib_status);	
	switch (mib_attribute) {
	...
	case DOT11_WEP_DEFAULT_KEY_VALUE1:
		//printk(KERN_INFO  "DOT11_WEP_DEFAULT_KEY_VALUE1:mib_status=%d\n",(int)mib_status);
	...
	case DOT11_WEP_DEFAULT_KEY_VALUE2:
		//printk(KERN_INFO  "DOT11_WEP_DEFAULT_KEY_VALUE2:mib_status=%d\n",(int)mib_status);
		if(priv->wpa.rsn_enabled)
	...
	case DOT11_WEP_DEFAULT_KEY_VALUE3:
		//printk(KERN_INFO  "DOT11_WEP_DEFAULT_KEY_VALUE3:mib_status=%d\n",(int)mib_status);
		if(priv->wpa.rsn_enabled)
	...
	case DOT11_WEP_DEFAULT_KEY_VALUE4:
		//printk(KERN_INFO  "DOT11_WEP_DEFAULT_KEY_VALUE4:mib_status=%d\n",(int)mib_status);
		if(!priv->wpa.rsn_enabled)
	...
	case DOT11_RSN_ENABLED:
		//printk(KERN_INFO  "DOT11_RSN_ENABLED:mib_status=%d\n",(int)mib_status);
	...
	case DOT11_RSN_CONFIG_MULTICAST_CIPHER:
		//printk(KERN_INFO  "DOT11_RSN_CONFIG_MULTICAST_CIPHER:mib_status=%d\n",(int)mib_status);
		hostif_sme_enqueue(priv, SME_RSN_MCAST_CONFIRM);
		break;
	case DOT11_RSN_CONFIG_UNICAST_CIPHER:
		//printk(KERN_INFO  "DOT11_RSN_CONFIG_UNICAST_CIPHER:mib_status=%d\n",(int)mib_status);
		hostif_sme_enqueue(priv, SME_RSN_UCAST_CONFIRM);
		break;
	case DOT11_RSN_CONFIG_AUTH_SUITE:
		//printk(KERN_INFO  "DOT11_RSN_CONFIG_AUTH_SUITE:mib_status=%d\n",(int)mib_status);
		hostif_sme_enqueue(priv, SME_RSN_AUTH_CONFIRM);
		break;
	case DOT11_PMK_TSC:
		//printk(KERN_INFO  "DOT11_PMK_TSC:mib_status=%d\n",(int)mib_status);
		break;
	case DOT11_GMK1_TSC:
		//printk(KERN_INFO  "DOT11_GMK1_TSC:mib_status=%d\n",(int)mib_status);
		if(atomic_read(&priv->psstatus.snooze_guard)){
			atomic_set(&priv->psstatus.snooze_guard, 0);
		}
		break;
	case DOT11_GMK2_TSC:
		//printk(KERN_INFO  "DOT11_GMK2_TSC:mib_status=%d\n",(int)mib_status);
		if(atomic_read(&priv->psstatus.snooze_guard)){
			atomic_set(&priv->psstatus.snooze_guard, 0);
		}
		break;
	case LOCAL_PMK:
		//printk(KERN_INFO  "LOCAL_PMK:mib_status=%d\n",(int)mib_status);
		break;
	case LOCAL_GAIN:
		//printk(KERN_INFO  "LOCAL_GAIN:mib_status=%d\n",(int)mib_status);
		break;
#ifdef WPS
	case LOCAL_WPS_ENABLE:
		//printk(KERN_INFO  "LOCAL_WPS_ENABLE:mib_status=%d\n",(int)mib_status);
		break;
	case LOCAL_WPS_PROBE_REQ:
		//printk(KERN_INFO  "LOCAL_WPS_PROBE_REQ:mib_status=%d\n",(int)mib_status);
		break;
#endif /* WPS */
	case LOCAL_REGION:
		//printk(KERN_INFO  "LOCAL_REGION:mib_status=%d\n",(int)mib_status);
		break;
	default :
		break;
	}
...
{
	//printk(KERN_INFO  "power confirm, powermgt:%u\n",priv->reg.powermgt);
...
{
	//printk(KERN_INFO  "hostif_sleep_confirm\n");
...
void hostif_start_confirm(ks_wlan_private *priv)
{
       ...
       if((priv->connect_status & CONNECT_STATUS_MASK)== CONNECT_STATUS){
               ...
               //printk(KERN_INFO  "IWEVENT: disconnect\n");
               wireless_send_event(priv->net_dev, SIOCGIWAP, &wrqu, NULL);
       }
#endif
	//printk(KERN_INFO  " scan_ind_count=%d\n",priv->scan_ind_count);
...
void hostif_connect_indication(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_connect_indication\n");	
	switch(connect_code){
	case RESULT_CONNECT: /* connect */
		//printk(KERN_INFO  "connect :: scan_ind_count=%d\n",priv->scan_ind_count);
		if(!(priv->connect_status & FORCE_DISCONNECT))
	...
	case RESULT_DISCONNECT: /* disconnect */
		//printk(KERN_INFO  "disconnect :: scan_ind_count=%d\n",priv->scan_ind_count);
		netif_carrier_off(netdev);
		tmp = FORCE_DISCONNECT & priv->connect_status;
		priv->connect_status = tmp + DISCONNECT_STATUS;
		break;
	default:
		//printk(KERN_INFO  "unknown connect_code=%d :: scan_ind_count=%d\n",
...
void hostif_stop_confirm(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_stop_confirm\n");
#ifdef _SDIO_
...
{
	//printk(KERN_INFO  "hostif_ps_adhoc_set_confirm\n");
...
void hostif_infrastructure_set_confirm(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_infrastructure_set_confirm\n");
	result_code = get_WORD(priv);
	//printk(KERN_INFO  "result code = %d\n",result_code);
...
{
	//printk(KERN_INFO  "hostif_adhoc_set_confirm\n");
	priv->infra_status = 1; /* infrastructure mode set */
	hostif_sme_enqueue(priv, SME_MODE_SET_CONFIRM);
}
...
void hostif_associate_indication(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_associate_indication\n");
	...
	//printk(KERN_INFO  "IWEVENT:ASSOCINFO\n");
	wireless_send_event(priv->net_dev, IWEVCUSTOM, &wrqu, buf);
#endif /* WIRELESS_EXT > 14 */
}
...
void hostif_bss_scan_confirm(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "result=%d :: scan_ind_count=%d\n", result_code, priv->scan_ind_count);
	...
	wrqu.data.length = 0;
	wrqu.data.flags = 0;
	//printk(KERN_INFO  "IWEVENT: SCAN CONFIRM\n");
...
void hostif_phy_information_confirm(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_phy_information_confirm\n");
	rssi = get_BYTE(priv);
	...
	//printk(KERN_INFO   "phyinfo confirm rssi=%d signal=%d\n", rssi, signal);
...
{
	//printk(KERN_INFO  "mic_failure=%u\n",priv->wpa.mic_failure.failure);
...
void hostif_event_check(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO   "++++++++++++hostif_event_check\n");
	event = get_WORD(priv); /* get event */
	//printf_hif_event(event);
	switch (event) {
	case HIF_DATA_IND:
	...
	case HIF_AP_SET_CONF:
	default:
		//printk(KERN_INFO   "undefined event[%04X]\n", event);
...
int hostif_data_request(ks_wlan_private *priv, struct sk_buff *packet)
{
	...
	if (packet_len > ETH_FRAME_LEN) {
		printk(KERN_INFO  "bad length packet_len=%d \n", packet_len);
	...
	if(((priv->connect_status & CONNECT_STATUS_MASK)== DISCONNECT_STATUS) ||
	   (priv->connect_status & FORCE_DISCONNECT) || priv->wpa.mic_failure.stop){
		...
		//printk(KERN_INFO  "data request packet_len=%d\n",packet_len);
	...
	//printk(KERN_INFO   "skb_buff length=%d\n", packet_len);
	...
	if (memcmp(&priv->eth_addr[0],eth->h_source, ETH_ALEN)){
		...
		printk(KERN_INFO   "ethernet->h_source=%02X:%02X:%02X:%02X:%02X:%02X\n",
			eth->h_source[0],eth->h_source[1],eth->h_source[2],
			eth->h_source[3],eth->h_source[4],eth->h_source[5]);
		return -3;
	}
		//printk(KERN_INFO   "valid mac address !!\n");
		//printk(KERN_INFO   "ethernet->h_source=%02X:%02X:%02X:%02X:%02X:%02X\n",
		//	eth->h_source[0],eth->h_source[1],eth->h_source[2],
		//	eth->h_source[3],eth->h_source[4],eth->h_source[5]);
	...
	if (*(buffer+1) + (*buffer << 8) > 1500) {
		/* ProtocolEAP = *(buffer+1) + (*buffer << 8); */
	        /* printk(KERN_INFO   "Send [SNAP]Type %x\n",ProtocolEAP); */
...
void hostif_mib_get_request( ks_wlan_private *priv, unsigned long mib_attribute)
{
	...
	//printk(KERN_INFO   "hostif_mib_get_request\n");
			     ...
			     void *vp )
{
	...
	//printk(KERN_INFO  "hostif_mib_set_request\n");

	if (priv->dev_state < DEVICE_STATE_BOOT) {
		printk(KERN_INFO  "DeviceRemove\n");
		return;
	}
...
void hostif_start_request( ks_wlan_private *priv, unsigned char mode )
{
	...
	//printk(KERN_INFO  "hostif_start_request\n");
...
void hostif_ps_adhoc_set_request(ks_wlan_private *priv)
{
	...
	uint16_t capability;
        ...
        //printk(KERN_INFO  "hostif_ps_adhoc_set_request\n");
...
void hostif_adhoc_set_request(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO   "hostif_adhoc_set_request\n");
...
void hostif_adhoc_set2_request(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO   " hostif_adhoc_set2_request\n");
...
void hostif_stop_request( ks_wlan_private *priv )
{
	...
	//printk(KERN_INFO  "hostif_stop_request\n");
...
void hostif_phy_information_request( ks_wlan_private *priv )
{
	...
	//printk(KERN_INFO  "hostif_phy_information_request\n");
				 ...
				 unsigned long wake_up, unsigned long receiveDTIMs )
{
	...
	//printk(KERN_INFO  "mode=%lu wake_up=%lu receiveDTIMs=%lu\n",mode,wake_up,receiveDTIMs);
	/* make primitive */
...
void hostif_sleep_request( ks_wlan_private *priv, unsigned long mode)
{
	...
	if(mode == SLP_SLEEP){
		...
		printk(KERN_INFO  "invalid mode %ld \n", mode);
		return;
	}
}
...
void hostif_bss_scan_request( ks_wlan_private *priv, unsigned long scan_type , uint8_t *scan_ssid, uint8_t scan_ssid_len)
{
	...
	//printk(KERN_INFO  " hostif_bss_scan_request\n");
	/* make primitive */
				 ...
				 unsigned short timer )
{
	...
	//printk(KERN_INFO  "count=%d :: timer=%d\n",failure_count,timer);
	/* make primitive */
...
{
	//printk(KERN_INFO  "hostif_receive hostif_receive\n");
...
void hostif_sme_mode_setup(ks_wlan_private *priv)
{
	...
	case MODE_INFRASTRUCTURE:
		...
		if (!is_valid_ether_addr((u8 *)priv->reg.bssid)) {
			...
			printk(KERN_INFO   "Infra bssid = %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	case MODE_ADHOC:
		...
		if (!is_valid_ether_addr((u8 *)priv->reg.bssid)) {
			...
			printk(KERN_INFO   "Adhoc bssid = %02x:%02x:%02x:%02x:%02x:%02x\n",
			       priv->reg.bssid[0],priv->reg.bssid[1],priv->reg.bssid[2],
    			   priv->reg.bssid[3],priv->reg.bssid[4],priv->reg.bssid[5]);
		}
		break;
...
void hostif_sme_multicast_set(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_sme_multicast_set\n");
...
//			for (i = 0; mclist && i < mc_count; i++, mclist = mclist->next)		
			netdev_for_each_mc_addr(mclist , dev)
				memcpy(&set_address[i*ETH_ALEN], mclist->addr, ETH_ALEN);
...
void hostif_sme_powermgt_set(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "hostif_sme_powermgt_set,priv->reg.powermgt: %u\n",priv->reg.powermgt);
...
{
	//printk(KERN_INFO  "hostif_sme_sleep_set\n");
...
void hostif_sme_set_pmksa(ks_wlan_private *priv)
{
	...
	//printk(KERN_INFO  "pmklist.size=%d\n",priv->pmklist.size);
	i=0;
...
void hostif_sme_execute(ks_wlan_private *priv, int event)
{
	...
	//printk(KERN_INFO  "hostif_sme_execute   event=%d\n",event);
	switch (event) {
	case SME_START:
		if ( priv->dev_state == DEVICE_STATE_BOOT ){
	...
	case SME_MIC_FAILURE_REQUEST:
		if(priv->wpa.mic_failure.failure == 1){
			...
			printk(KERN_INFO  "SME_MIC_FAILURE_REQUEST: failure count=%u error?\n",
				priv->wpa.mic_failure.failure);
		break;
...
void hostif_sme_task( unsigned long dev )
{
	...
	//printk(KERN_INFO  "hostif_sme_task\n");
...
{
	//printk(KERN_INFO  "hostif_sme_enqueue\n");
	...
	if(atomic_read(&priv->psstatus.status)==PS_SNOOZE && event < SME_START_CONFIRM){ /* power save wakeup*/
		...
		if(atomic_read(&priv->sme_task.count) <= 0){ 
			/* schedule_work(&priv->ks_wlan_wakeup_task); */
			//printk(KERN_INFO  "sme task disable.\n");
	...
	if (cnt_smeqbody(priv) < (SME_EVENT_BUFF_SIZE - 1)) {
		...
		//printk(KERN_INFO  "sme queue buffer overflow\n");
