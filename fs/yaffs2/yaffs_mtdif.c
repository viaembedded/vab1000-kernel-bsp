				   ...
				   const u8 *oob, int oob_len)
{
	...
	if (retval) {
		yaffs_trace(YAFFS_TRACE_MTD,
			"write_oob failed, chunk %d, mtd error %d",nand_chunk, retval);
	}
	LEAVE();
				   ...
				   enum yaffs_ecc_result *ecc_result)
{
	...
	if (retval)
		yaffs_trace(YAFFS_TRACE_MTD,
			"read_oob failed, chunk %d, mtd error %d",nand_chunk, retval);
...
struct mtd_info * yaffs_get_mtd_device(dev_t sdev)
{
	...
	if (mtd->type != MTD_NANDFLASH) {
		yaffs_trace(YAFFS_TRACE_ALWAYS,"yaffs: MTD device is not NAND it's type %d",mtd->type);
		LEAVE();
		return NULL;
	}
